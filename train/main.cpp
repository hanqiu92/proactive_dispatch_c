...
int main(int argc, const char * argv[]) {
    if (argc != 8){
        ...
        if (i_algo == 0) algo = Algorithm::assort_adjust;
        if (i_algo == 1) algo = Algorithm::pricing_adjust;
        ...
        if (debug_flag > 0){
            std::cout<<"setting: "<<congestion_factor<<","<<demand_factor<<","<<supply_factor<<","<<p_rate<<","<<tax_congest<<","<<tax_demand<<",";
