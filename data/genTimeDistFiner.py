...
pair = pair.as_matrix()
t_dist = np.zeros((48,10,10))
n_dist = np.zeros((48,10,10))
for i in xrange(pair.shape[0]):
    ...
    d_x = int(pair[i,2])
    d_y = int(pair[i,3])
    t_dist[(t-1)/30,o_x-1,o_y-1] += dt
    t_dist[(t-1)/30,d_x-1,d_y-1] += dt
    n_dist[(t-1)/30,o_x-1,o_y-1] += 1.0
    n_dist[(t-1)/30,d_x-1,d_y-1] += 1.0
...
def fill(seq):
    if sum(seq) < 1.0:
        ...
        if ((ind[-1] - ind[0]) < 47):
            ...
            x = seq[ind[-1]]
            y = seq[ind[0]]
            for i in xrange(ind[-1],47+ind[0]):
                seq[i%47] = (x * (b-i) + y * (i-a)) / float(b-a)
...
pickle.dump(t_dist, open( "t_dist_finer.p", "wb" ))
